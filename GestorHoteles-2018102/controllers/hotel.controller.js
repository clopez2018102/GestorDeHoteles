...
function saveHotel(req, res){
    ...
    if( params.name &&
        params.num &&
        params.password){
            Hotel.findOne({$or:[{'data.name': params.name}, {'num': params.num}]}, (err, hotelFind)=>{
                if(err){
                    res.status(500).send({message: 'Error general, intentelo mas tarde'})
                }else if(hotelFind){
                    res.send({message: 'Nombre o num ya utilizado'});
                }else{
                    hotel.data.name = params.name;
                    hotel.num = params.num;
...
function login(req, res){
    ...
    if(params.num && params.password){
            Hotel.findOne({num: params.num}, (err, check)=>{
                    if(err){
